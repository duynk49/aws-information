...
import aws_utils
from modules import billing, config, infra
...
def setup_parser():
    ...
    billing_parser.add_argument(
        "--month", "-m", help="Month in YYYY-MM format (default: current month)"
    )
    ...
    config_parser.add_argument("--account", "-a", help="AWS account name to operate on")
    ...
    config_parser.add_argument(
        "--output", "-o", help="Custom output filename (without extension)"
    )

    # Create nested subparsers for config management commands
    config_subparsers = config_parser.add_subparsers(
        dest="config_action", help="Configuration management commands", metavar="ACTION"
    )

    # add-account subcommand
    add_account_parser = config_subparsers.add_parser(
        "add-account", help="Add a new AWS account to configuration"
    )
    add_account_parser.add_argument(
        "--account-id", required=True, help="AWS Account ID"
    )
    add_account_parser.add_argument("--name", required=True, help="Account name/alias")
    add_account_parser.add_argument(
        "--services", required=True, help="Comma-separated list of services"
    )

    # update-account subcommand
    update_account_parser = config_subparsers.add_parser(
        "update-account", help="Update existing account configuration"
    )
    update_account_parser.add_argument(
        "--account-id", required=True, help="AWS Account ID"
    )
    update_account_parser.add_argument(
        "--add-services", help="Comma-separated services to add"
    )
    update_account_parser.add_argument(
        "--remove-services", help="Comma-separated services to remove"
    )
    update_account_parser.add_argument(
        "--set-services",
        help="Comma-separated services to replace all current services",
    )

    # add-service subcommand
    add_service_parser = config_subparsers.add_parser(
        "add-service", help="Add a new AWS service configuration"
    )
    add_service_parser.add_argument("--service", required=True, help="Service name")
    add_service_parser.add_argument(
        "--commands", required=True, help="Comma-separated list of AWS CLI commands"
    )

    # list-accounts subcommand
    config_subparsers.add_parser("list-accounts", help="List all configured accounts")

    # list-services subcommand
    config_subparsers.add_parser("list-services", help="List all configured services")
...
def main():
    ...
    try:
        ...
        print(f"❌ AWS Authentication failed: {e!s}")
        sys.exit(1)
    ...
    if result["success"]:
        ...
        if "stats" in result:
            ...
            if "success_rate" in stats:
                print(
                    f"📊 Success Rate: {stats['success_rate']:.1f}% ({stats['successful_commands']}/{stats['total_commands']})"
                )
            elif "accounts_analyzed" in stats:
                print(
                    f"📊 Analysis: {stats['accounts_analyzed']} accounts, {stats['billing_files_processed']} files, {stats['unmapped_services']} unmapped services"
                )
    else:
        print(
