...
def handle_aws_authentication(skip_login=False):
    ...
    if not skip_login:
        ...
        print("⚠️  Skipping login. Checking existing credentials...")
        credentials_valid, account_info = check_aws_credentials()
        if not credentials_valid:
...
def setup_common_parser(account_required=False, account_help=None):
    ...
    parser = argparse.ArgumentParser(
        add_help=False
    )  # Don't add help so it can be inherited
    ...
    parser.add_argument("--account", "-a", required=account_required, help=account_help)
    parser.add_argument(
        "--skip-login",
        action="store_true",
...
def get_output_filename(account_name, output_arg=None, file_extension=".json"):
    ...
    if output_arg:
        filename = (
            output_arg
            if output_arg.endswith(file_extension)
            else f"{output_arg}{file_extension}"
        )
    else:
...
def get_flat_storage_filename(
    data_type, date_str, account_name, account_id, output_arg=None
):
    """
