    ...
    try:
        ...
        if result.returncode == 0:
            try:
                ...
                error_msg = f"Invalid JSON response for {command_name}: {str(e)}"
                if error_logger:
                    error_logger.error(f"JSON_DECODE_ERROR - Command: {command_name}, AWS_Command: {command}, Error: {error_msg}\nStdout: {result.stdout[:500]}...")
            ...
            if "InvalidParameterValue" in error_msg and "time period" in error_msg:
                ...
                return {"data": {"note": "No billing data available for specified time period"}, "success": True}
            elif "AccessDeniedException" in error_msg and "ce:" in error_msg:
                # Cost Explorer permissions missing
                return {"data": {"note": "Cost Explorer access not enabled (normal for many accounts)"}, "success": True}
            elif "ValidationException" in error_msg and "granularity" in error_msg:
                # Cost Explorer granularity validation issues
                return {"data": {"note": "Invalid granularity for time period (expected for short ranges)"}, "success": True}
            else:
                if error_logger:
                    error_logger.error(f"AWS_BILLING_COMMAND_FAILED - Command: {command_name}, AWS_Command: {command}, ReturnCode: {result.returncode}, Error: {error_msg}")
                return {"error": error_msg, "success": False}
            
    except subprocess.TimeoutExpired:
        error_msg = "Command timeout after 120 seconds"
        if error_logger:
            error_logger.error(f"BILLING_TIMEOUT_ERROR - Command: {command_name}, AWS_Command: {command}, Error: {error_msg}")
        return {"error": error_msg, "success": False}
    except Exception as e:
        error_msg = f"Failed to execute command: {str(e)}"
        if error_logger:
            error_logger.error(f"BILLING_EXECUTION_ERROR - Command: {command_name}, AWS_Command: {command}, Error: {error_msg}\nTraceback: {traceback.format_exc()}")
        return {"error": error_msg, "success": False}
...
def run_billing(account_info, args):
    ...
    if hasattr(args, 'month') and args.month:
        try:
            ...
            print(f"Error: Month format must be YYYY-MM (e.g., 2026-03)")
    ...
    account_name_for_display = aws_utils.determine_account_name(account_info, args.account, ACCOUNT_CONFIGS)
    ...
    if not account_found:
        ...
        account_names = [config.get('name') for config in ACCOUNT_CONFIGS.values() if config.get('name')]
    ...
    filename = aws_utils.get_flat_storage_filename("billing", date_str, account_name, found_account_id, args.output)
    ...
    for command_name, command in billing_commands.items():
        ...
        if result["success"]:
            ...
            if command_name == "monthly_cost_by_service" and "ResultsByTime" in result["data"]:
                total_cost = 0
                for time_period in result["data"]["ResultsByTime"]:
                    if "Total" in time_period and "UnblendedCost" in time_period["Total"]:
                        try:
                            cost = float(time_period["Total"]["UnblendedCost"]["Amount"])
                            total_cost += cost
                        except (ValueError, KeyError):
                            pass
                final_output["billing_data"][command_name]["total_monthly_cost"] = total_cost
            ...
            if "AccessDenied" in error_msg or "UnauthorizedOperation" in error_msg:
                print(f"   🔒 {command_name}: Access denied - check Cost Explorer permissions")
            ...
            elif "ValidationException" in error_msg and "end date past the beginning" in error_msg:
                print(f"   📅 {command_name}: Date range issue (future month data not available)")
            elif "There is no available data" in error_msg:
                print(f"   📊 {command_name}: No data available for the requested time period")
    ...
    print(f"   📈 Success Rate: {(successful_commands/final_output['summary']['total_commands']*100):.1f}%")
    ...
    return {
        ...
        "stats": {
            ...
            "success_rate": (successful_commands/final_output['summary']['total_commands']*100) if final_output['summary']['total_commands'] > 0 else 0
        },
        "billing_period": {
            "start_date": start_date,
            "end_date": end_date,
