...
class ConfigManager:
    ...
    def add_account(self, account_id, name, services):
        ...
        config["account_configs"][account_id] = {"name": name, "services": services}

        self.save_accounts(config)
        print(
            f"✅ Added account '{name}' (ID: {account_id}) with services: {', '.join(services)}"
        )

    def update_account(
        self, account_id, add_services=None, remove_services=None, set_services=None
    ):
        ...
        current_services = set(
            account_config.get("services", [])
        )  # Handle missing services field
        ...
        account_config["services"] = sorted(new_services)
        self.save_accounts(config)
    ...
    def validate(self):
        ...
        try:
            ...
            for account_id, account_config in accounts_config[
                "account_configs"
            ].items():
                ...
                if not services:
                    issues.append(
                        f"Account {account_id} ({account_config.get('name', 'unnamed')}) has no services configured"
                    )
                    continue
                ...
                for service in services:
                    if service not in available_services:
                        issues.append(
                            f"Account {account_id} uses undefined service '{service}'"
                        )
            ...
            print(
                f"   📊 {len(accounts_config['account_configs'])} accounts configured"
            )
...
class BillingAnalyzer:
    ...
    def get_billing_files(self, account_name=None, month=None):
        ...
        for file in aws_dir.glob(billing_pattern):
            ...
            if len(parts) >= 4 and parts[0] == "billing":
                ...
                file_account_name = "_".join(
                    parts[2:-1]
                )  # Account name (may contain underscores)
    ...
    def extract_services_from_billing(self, billing_file):
        ...
        services_found = []
        groups = (
            data.get("billing_data", {})
            .get("monthly_cost_by_service", {})
            .get("ResultsByTime", [])
        )
    ...
    def analyze_billing_files(self, files):
        ...
        for billing_file in files:
            ...
            account_name, month, services = self.extract_services_from_billing(
                billing_file
            )
            ...
            for service in services:
                service_key, original_name, _cost = service
                analysis_results[account_name]["all_services"].add(service_key)

                # Track unmapped services
                if (
                    service_key == original_name
                    and service_key not in self.service_mapping.values()
                ):
                    analysis_results[account_name]["unmapped_services"].add(service_key)
                    unmapped_services.add(service_key)

            print(
                f"   Found {len(services)} services with costs >= ${self.min_cost_threshold}"
            )
    ...
    def generate_service_commands(self, service_key):
        ...
        service_commands = {
            ...
            "waf": ["aws waf list-web-acls", "aws waf list-rules"],
            "securityhub": [
            ...
            "guardduty": ["aws guardduty list-detectors", "aws guardduty get-findings"],
        }

        return service_commands.get(
            service_key,
            [
    ...
    def print_analysis_report(self, analysis_results, unmapped_services):
        ...
        try:
            ...
            self.config_manager.load_commands()
        except Exception as e:
        ...
        for account_name, data in analysis_results.items():
            ...
            if account_id:
                ...
                print("   ID: Not found in configuration")
            print()
            ...
            for service in found_services:
                mapped_status = (
                    "✅" if service in self.service_mapping.values() else "❓"
                )
            ...
            if account_id and account_id in current_config.get("account_configs", {}):
                current_services = set(
                    current_config["account_configs"][account_id].get("services", [])
                )
                ...
                if config_only:
                    print(
                        f"   ⚠️  Services in config but not in billing ({len(config_only)}):"
                    )
                ...
                if billing_only:
                    print(
                        f"   ➕ Services in billing but not in config ({len(billing_only)}):"
                    )
                ...
                if both:
                    ...
                    print("   📝 Account has no services configured yet")
                    print()
    ...
    def auto_update_configurations(self, analysis_results, dry_run=False):
        ...
        for account_name, data in analysis_results.items():
            ...
            if not account_id:
                print(
                    f"⚠️  Skipping {account_name}: Account ID not found in configuration"
                )
                continue
            ...
            if mapped_services:
                if dry_run:
                    print(
                        f"📝 Would add services to {account_name}: {', '.join(sorted(mapped_services))}"
                    )
                else:
                    print(
                        f"➕ Adding services to {account_name}: {', '.join(sorted(mapped_services))}"
                    )
                    self.config_manager.update_account(
                        account_id, add_services=list(mapped_services)
                    )
            ...
            for service in unmapped_services:
                if service not in available_services:
                    ...
                    if dry_run:
                        print(
                            f"📝 Would add service definition for {service} with {len(commands)} commands"
                        )
                    else:
...
def prompt_add_account(account_id, account_info, config_manager):
    ...
    while True:
        response = (
            input(
                "\n➕ Would you like to add this account to the configuration? (y/n): "
            )
            .strip()
            .lower()
        )
    ...
    # Get account name
    default_name = (
        account_info["account_alias"]
        if account_info["account_alias"]
        else f"account-{account_id}"
    )
    while True:
        account_name = input(
            f"📝 Enter a name for this account (default: {default_name}): "
        ).strip()
        if not account_name:
    ...
    print("💡 Or press Enter for a default set: ec2, s3, iam, vpc, cloudwatch")

    services_input = input("🔧 Enter services (comma-separated): ").strip()
    ...
    try:
        ...
        if account_info:
            ...
            account_name = aws_utils.get_account_name_by_id(
                accounts_config.get("account_configs", {}), account_info["account_id"]
            )

        # Get billing files to analyze (latest month if no specific month provided)
        billing_files = analyzer.get_billing_files(
            account_name=account_name, month=getattr(args, "month", None)
        )
        ...
        analysis_results, unmapped_services = analyzer.analyze_billing_files(
            billing_files
        )
        ...
        update_success = analyzer.auto_update_configurations(
            analysis_results, dry_run=dry_run
        )
        ...
        return {"success": False, "error": f"Configuration workflow failed: {e!s}"}
...
def handle_config_management(args):
    ...
    try:
        if args.config_action == "add-account":
            ...
            add_services = (
                [s.strip() for s in args.add_services.split(",")]
                if args.add_services
                else None
            )
            remove_services = (
                [s.strip() for s in args.remove_services.split(",")]
                if args.remove_services
                else None
            )
            set_services = (
                [s.strip() for s in args.set_services.split(",")]
                if args.set_services
                else None
            )
            ...
            if success:
                return {
                    "success": True,
                    "message": f"Updated account '{args.account_id}'",
                }
            else:
                return {
                    "success": False,
                    "error": f"Account '{args.account_id}' not found",
                }
            ...
            return {
                "success": False,
                "error": f"Unknown config action: {args.config_action}",
            }

    except Exception as e:
        return {"success": False, "error": f"Config management error: {e!s}"}
