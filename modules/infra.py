    ...
    try:
        ...
        if result.returncode == 0:
            try:
                ...
                if not output:
                    return {"data": {"note": "Empty response (no data available)"}, "success": True}
                return {"data": json.loads(output), "success": True}
            except json.JSONDecodeError as e:
                error_msg = f"Invalid JSON response for {service_name}: {str(e)}"
                error_logger.error(f"JSON_DECODE_ERROR - Service: {service_name}, Command: {command}, Error: {error_msg}\nStdout: {result.stdout[:500]}...")
            ...
            if "NoSuchEntity" in error_msg and "Password Policy" in error_msg:
                ...
                return {"data": {"note": "No password policy configured (this is normal)"}, "success": True}
            elif (
                ...
                return {"data": {"note": "Cache security groups not available in VPC (expected)"}, "success": True}
            elif (
                "WAFInvalidParameterException" in error_msg
                and "CLOUDFRONT" in error_msg
            ):
                # CloudFront WAF only available in us-east-1
                return {"data": {"note": "CloudFront WAF scope only available in us-east-1"}, "success": True}
            elif "ClusterNotFoundException" in error_msg and service_name == "ecs":
                # No ECS clusters is normal for many accounts
                return {"data": {"note": "No ECS clusters found (normal if account doesn't use ECS)"}, "success": True}
            elif "NoSuchTagSet" in error_msg:
                # S3 bucket has no tags - this is normal
                return {"data": {"note": "No tags configured for this resource (normal)"}, "success": True}
            elif "NoSuchBucketPolicy" in error_msg:
                # S3 bucket has no policy - this is normal
                return {"data": {"note": "No bucket policy configured (normal)"}, "success": True}
            else:
                error_logger.error(f"AWS_COMMAND_FAILED - Service: {service_name}, Command: {command}, ReturnCode: {result.returncode}, Error: {error_msg}")
                return {"error": error_msg, "success": False}

    except subprocess.TimeoutExpired:
        error_msg = "Command timeout after 60 seconds"
        error_logger.error(f"TIMEOUT_ERROR - Service: {service_name}, Command: {command}, Error: {error_msg}")
        return {"error": error_msg, "success": False}
    except Exception as e:
        error_msg = f"Failed to execute command: {str(e)}"
        error_logger.error(f"EXECUTION_ERROR - Service: {service_name}, Command: {command}, Error: {error_msg}\nTraceback: {traceback.format_exc()}")
        return {"error": error_msg, "success": False}
...
def get_dynamic_commands(service, initial_data, error_logger):
    ...
    try:
        ...
        error_logger.error(f"DYNAMIC_COMMAND_ERROR - Service: {service}, Error: {error_msg}\nTraceback: {traceback.format_exc()}")
        print(f"      ⚠️  {error_msg}")
...
def handle_command_error(error_msg, service, operation):
    ...
    if "Invalid JSON response" in error_msg and "Expecting value: line 1 column 1" in error_msg:
        ...
        # No ECS clusters is normal for many accounts
        return {
            "data": {"note": "No ECS clusters found (normal if account doesn't use ECS)"}, 
        ...
        return {
            "data": {"note": "Listeners require load balancer ARN - moved to dynamic discovery"}, 
...
def run_infra(account_info, args):
    ...
    try:
        account_name = aws_utils.determine_account_name(account_info, args.account, ACCOUNT_CONFIGS)
        if not account_name:
            error_logger.error(f"Could not determine account name for account_info: {account_info}, args.account: {args.account}")
            print("❌ Could not determine account name.")
            return {"success": False, "error": "Could not determine account name"}
    except Exception as e:
        error_logger.error(f"Failed to determine account name: {str(e)}\nTraceback: {traceback.format_exc()}")
        return {"success": False, "error": f"Failed to determine account name: {str(e)}"}
    ...
    filename = aws_utils.get_flat_storage_filename("infrastructure", date_str, account_name, account_id, args.output)
    ...
    if account_id in ACCOUNT_CONFIGS:
        ...
        print("💡 Please add this account ID to configs/accounts.yaml for full configuration")
    ...
    for service, commands in commands_map.items():
        ...
        for i, command in enumerate(commands):
            ...
            if result["success"]:
                ...
                # Handle known "expected" errors more gracefully for initial commands too
                error_handling = handle_command_error(result["error"], service, operation)
                
                if error_handling["is_error"]:
                    final_output["services"][service]["errors"][operation] = result["error"]
                    error_logger.error(f"OPERATION_FAILED - Service: {service}, Operation: {operation}, Error: {result['error']}")
                    print(f"      ❌ {operation}: {error_handling['display']}")
                else:
                    final_output["services"][service]["data"][operation] = error_handling["data"]
        ...
        if dynamic_commands:
            print(f"   🔄 Executing {len(dynamic_commands)} dynamic discovery commands...")
            ...
            for dynamic_command in dynamic_commands:
                ...
                if result["success"]:
                    ...
                    if error_handling["is_error"]:
                        final_output["services"][service]["errors"][operation] = result["error"]
                        error_logger.error(f"DYNAMIC_OPERATION_FAILED - Service: {service}, Operation: {operation}, Command: {dynamic_command}, Error: {result['error']}")
    ...
    try:
        ...
        error_logger.info(f"Successfully completed AWS resource collection for account: {account_name}")
        error_logger.info(f"Output written to: {output_path}")
        error_logger.info(f"Total commands: {total_commands}, Successful: {successful_commands}, Failed: {failed_commands}")
    except Exception as e:
        error_logger.error(f"WRITE_ERROR - Failed to write output file {output_path}: {str(e)}\nTraceback: {traceback.format_exc()}")
    ...
    if failed_services:
        print(f"❌ Failed services ({len(failed_services)}): {', '.join(failed_services)}")
        print("💡 Check AWS permissions and service availability")

    # Write error summary to log for AI analysis
    if failed_commands > 0:
        error_logger.info(f"ERROR_SUMMARY_START - Account: {account_name}, Failed Commands: {failed_commands}")
        for service_name, service_data in final_output["services"].items():
            if "errors" in service_data:
                for operation, error in service_data["errors"].items():
                    error_logger.info(f"ERROR_DETAIL - Service: {service_name}, Operation: {operation}, Error: {error}")
